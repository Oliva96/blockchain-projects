...
contract AuctionCreator{
    ...
    function createAuction(uint _blocksDuration, uint _highestBid) public{
        
        // passing msg.sender to the constructor of Auction 
        Auction newAuction = new Auction(payable(msg.sender), _blocksDuration, _highestBid); 
...
contract Auction{
    ...
    constructor(address payable _eoa, uint _blocksDuration, uint _highestBid){
        owner = _eoa;
    ...
    function finalizeAuction() public{
       ...
       if(auctionState == State.Canceled && msg.sender != owner){ // auction canceled, not ended
           ...
           value = bids[msg.sender];
            ...
            if(auctionState == State.Ended){
                if(msg.sender == owner && ownerFinalized == false){ //the owner finalizes the auction
                    ...
                    if (msg.sender != highestBidder){ //this is neither the owner nor the highest bidder (it's a regular bidder)
                        recipient = payable(msg.sender);
                        value = bids[msg.sender];
                    }
                }
